...
func (w *WasmIntptr) StateDB() state.StateDB {
...
func (w *WasmIntptr) verifyModule(m *wasm.Module) (int, error) {
	...
	// Validate whether the function imported from `sea` module are in the list of eei_api or not
	if m.Import != nil {
		for _, entry := range m.Import.Entries {
			if entry.ModuleName == "sea" && entry.Type.Kind() == wasm.ExternalFunction {
